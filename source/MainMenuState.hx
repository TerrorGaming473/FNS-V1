...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bar:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('BARmain'));
		bar.updateHitbox();
		bg.x = 1280;
		bg.screenCenter(Y);
		...
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "Friday Night Streets' v" + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		...
		#end
		FlxTween.tween(menuItem, { x:400 }, 1.5, {ease});
